...
First, the mass-split produced by the truncation routine is checked visually and quantitatively: the inner and outer mass components sum to the original halo mass and the outer component vanishes beyond the chosen cutoff radius. The mass-split diagnostic is shown in Figure~\ref{fig:tnfw_validation_mass_split} and confirms that mass is correctly assigned to the inner particle region and to the truncated outer component.
...
In summary, the validation diagnostics show that (i) mass is correctly split by the truncation routine (Figure~\ref{fig:tnfw_validation_mass_split}), (ii) the inner potential follows the expected NFW-like shape and tracks the baseline within the inner few kiloparsecs (Figure~\ref{fig:tnfw_validation_potential_multiscale}), and (iii) truncation progressively suppresses long-range contributions as $R_{\mathrm{cut}}$ decreases (Figure~\ref{fig:multifigure_potential_distr}).

\subsection{Orbits crossing the truncation boundary}

Selected test particles that cross the truncation boundary were integrated to check for spurious energy or angular-momentum transfer caused by the hybrid treatment. Example orbits are shown in Figure~\ref{fig:tnfw_validation_orbits} and remain regular and physically plausible, indicating the hybrid coupling does not introduce obvious integration artefacts on the tested timescales.
...
These outcomes give us confidence that the truncated interaction implementation behaves as intended for the described experiments.
