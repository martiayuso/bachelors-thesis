...
More orbit plots can be found in the annex Section~\ref{app:tnfw_orbits}. A larger number of orbits is summarized in the following plot. In all cases, the orbits remain regular and show no inconsistencies when crossing the truncation boundary.
...
In summary, the validation diagnostics show that (i) mass is correctly split by the truncation routine, (ii) the inner potential follows the expected NFW-like shape and tracks the baseline within the inner few kiloparsecs (Figure~\ref{fig:multifigure_potential_distr}), and (iii) orbits are not affected by the truncation (Figure~\ref{fig:summary_orbits}). These outcomes indicate that the truncated interaction implementation behaves as intended for the described experiments.

\subsection{Density profile}
A complete 100 Gyr run of the density evolution for Rcut = 10 kpcs is shown here. The plot shows an evolution parallel to the baseline featured in section \ref{sec:results_baseline}, with a similar secular decline in the central density. This is a key result because it suggests that the mechanism behind the decline does not require the full long-range potential structure of the halo, and that truncating interactions beyond 10 kpc does not eliminate the effect.
...
This motivates further analysis of smaller Rcut values, which is carried out in the following plot.
