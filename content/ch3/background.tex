...
SWIFT is a highly-parallel, open-source astrophysics and cosmology simulation code specifically designed to exploit modern high-performance computing (HPC) and cluster architectures \citep{schaller2024swift}. Unlike traditional simulation codes that distribute workloads by strictly decomposing spatial data across processors, SWIFT relies on a hybrid shared- and distributed-memory task-based parallelism model \citep{schaller2024swift}. This architecture utilizes asynchronous communications and advanced domain-decomposition algorithms that dynamically balance the computational workload rather than the data footprint. This approach minimizes synchronization bottlenecks and ensures excellent weak-scaling performance, allowing the code to efficiently handle simulations involving hundreds of billions of particles \citep{nasar2025task,schaller2024swift}.
...
\subsection{Smoothed particle hydrodynamics (SPH) and the SPHENIX scheme}

While this thesis primarily investigates the dynamics of collisionless dark matter halos, a complete theoretical overview of SWIFT requires acknowledging its capacity to model baryonic physics through hydrodynamics. SWIFT accomplishes this using Smoothed Particle Hydrodynamics (SPH), a Lagrangian (particle-based) method that discretizes fluids into individual elements that move naturally with the flow \citep{kegerreis2019planetary,nasar2025task}. SPH is highly favored in cosmology due to its intrinsic spatial adaptivity; it effortlessly clusters resolution elements into high-density regions, seamlessly capturing the extreme density contrasts inherent to structure formation without the need for complex adaptive mesh refinement algorithms \citep{nasar2025task}.

SWIFT features multiple flavors of SPH, but its default and most advanced scheme is SPHENIX \citep{borrow2020sphenix,schaller2024swift}. SPHENIX is a density-energy based SPH formulation engineered to overcome the historical integration errors and mixing limitations associated with traditional pressure-entropy SPH methods \citep{borrow2020sphenix}. To achieve this, SPHENIX introduces a novel artificial conduction scheme alongside standard artificial viscosity. This addition significantly reduces errors at contact discontinuities and dramatically improves both the conservation of vorticity and the mixing of fluids \citep{borrow2020sphenix}. By combining robust task-based gravity with the high-fidelity SPHENIX hydrodynamics scheme, SWIFT provides an optimal numerical framework for executing the next generation of highly accurate galaxy formation simulations \citep{borrow2020sphenix,schaller2024swift}.

\section{Shell structure and mass redistribution}
\label{sec:bg_shells}
