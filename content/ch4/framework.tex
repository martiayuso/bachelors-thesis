...
\end{equation}
with minimum and maximum timesteps constrained in the interval
\begin{equation}
...
Hybrid model & Cusp & $\mathbf{p}=\mathbf{p}_0$, $R_{\mathrm{cut}}\in\{[0.01,\,5],10,25,50\}$ kpc & 20 & $5 \times 10^3$ \\
\hline
\end{tabular}
}
\end{table}
...
\(m_{\rm stored} = m_{\rm initial}\times\varepsilon_m\) for dark matter particles with $r>R_{\rm cut}$, where $\varepsilon_m\approx10^{-25}$. After this one-time pass, the stored \texttt{gp->mass} carries the masked value, and the rest of the gravity pipeline consumes that stored mass in the usual way during multipole builds, pair interactions and mesh depositions.
...
for i in 0..nr_gparts-1:
	...
	if gp.type == DM and DM_masking_enabled and |gp.x - centre|^2 > R_cut^2:
		...
		gp.mass = epsilon_m * gp.mass_initial	# overwrite stored mass
	else:
		gp.mass_initial = gp.mass	# ensure mass_initial is set consistently
end
\end{lstlisting}

\item \textbf{Per-step gravity workflow}
\begin{lstlisting}[basicstyle=\footnotesize\ttfamily,breaklines=true]
# Tree/multipole build
...
# Cache and write-back (target-side suppression)
...
\end{equation}
with $\Phi_{\mathrm{res}}$ the resolved potential and $\Phi_{\mathrm{ext}}$ the analytic truncated-NFW contribution. If strict continuity at $R_{\mathrm{cut}}$ is desired, the external backend can be shifted so that the potential remains continuous across the cutoff; this does not affect the force, but it does change the reported potential energy zero-point.
...
\subsection{Serialization, restart safety and MPI consistency}
...
To keep the hybrid masking consistent across MPI boundaries, the packed foreign gravity data carries the stored particle mass directly (i.e. the code exports `packed->mass = gp->mass`, which will be the masked tiny value for outer DM after the startup sweep). In practice, this means the same masked mass is used on every rank when building multipoles and evaluating pairwise gravity, without reconstructing mask factors on the receiver side.
...
\item masking is performed as a one-time startup/restart sweep that overwrites stored particle masses for outer DM; particles that start outside $R_{\mathrm{cut}}$ therefore remain masked even if they later move inward.
\end{itemize}
