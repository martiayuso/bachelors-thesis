...
The same logic can be written in compact pseudocode as follows, where the source and target operators are separated explicitly:

\begin{lstlisting}[basicstyle=\footnotesize\ttfamily,breaklines=true]
for each particle p:
	x_p = position(p)
	if type(p) == DM and DM_masking_enabled and |x_p - x_c|^2 > R_cut^2:
		m_eff(p) = epsilon_m * m(p)
	else:
		m_eff(p) = m(p)

for each gravity target t:
	x_t = position(t)
	if |x_t - x_c|^2 > R_cut^2:
		suppress resolved write-back: a_res(t) = 0, Phi_res(t) = 0
		a_ext(t) = a_NFW(x_t - x_c)
		Phi_ext(t) = Phi_NFW(x_t - x_c) + C_cut
	else:
		a_ext(t) = 0, Phi_ext(t) = 0
		keep resolved self-gravity only

diagnostics:
	E_pot,self = sum_i m_i * Phi_res(x_i)
	E_pot,ext  = sum_i m_i * Phi_ext(x_i)
\end{lstlisting}
