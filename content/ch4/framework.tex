...
The workflow combines (i) controlled collisionless $N$-body runs in SWIFT, (ii) parameter variations of the self-gravity solver, (iii) profile diagnostics extracted from snapshots, and (iv) shell-based analyses designed to quantify non-local contributions to the dynamics of the innermost region.
...
\subsection{Simulation campaign summary}
...
\begin{equation}
\mathbf{p}_0 = \{\eta=2\times10^{-3},\ \mathrm{MAC}=\mathrm{geometric},\ \theta_{\mathrm{cr}}=0.7,\ \epsilon_{\mathrm{fmm}}=10^{-3},\ \epsilon_{\mathrm{soft}}=0.01\,\mathrm{kpc}\},
\end{equation}
...
Box-size & Cuspy & $\mathbf{p}=\mathbf{p}_0$, $L_{\mathrm{box}}\in\{L_{\mathrm{s}},L_{\mathrm{m}},L_{\mathrm{l}}\}$ & 3 & n/a \\

Initial structure & Cuspy, core & $\mathbf{p}=\mathbf{p}_0$, IC$\in\{$cuspy, core$\}$ & 2 & n/a \\

Softening & Cuspy & $\mathbf{p}=\mathbf{p}_0$, $\epsilon\in\{1,5,10,50,100\}$ pc & 5 & 100 \\

Solver: timestep & Cuspy & $\mathbf{p}=\mathbf{p}_0$ with $\eta\in\{2\times10^{-4},2\times10^{-3},2\times10^{-2}\}$ & 3 & $10^2$ \\

Solver: MAC & Cuspy & $\mathbf{p}=\mathbf{p}_0$ with MAC$\in\{$geometric, adaptive$\}$ & 2 & n/a \\

Solver: opening angle & Cuspy & $\mathbf{p}=\mathbf{p}_0$ with $\theta_{\mathrm{cr}}\in\{0.5,0.6,0.7\}$ (geometric MAC) & 3 & 1.4 \\

Solver: FMM tolerance & Cuspy & $\mathbf{p}=\mathbf{p}_0$ with $\epsilon_{\mathrm{fmm}}\in\{10^{-4},10^{-3},10^{-2},10^{-1}\}$ & 4 & $10^3$ \\

Hybrid truncated-NFW & Cuspy & $\mathbf{p}=\mathbf{p}_0$, model$\rightarrow$hybrid, $R_{\mathrm{cut}}\in\{50,25,10,5,3,2,1\}$ kpc + vanilla & 8 & 50 \\
\hline
\end{tabular}
}
\end{table}

This representation makes the campaign auditable at a glance: all families are written as controlled perturbations of $\mathbf{p}_0$, with sweep spans of $10^2$ in $\eta$, $10^3$ in $\epsilon_{\mathrm{fmm}}$, $10^2$ in softening around the baseline $\epsilon_{\mathrm{soft}}=0.01\,\mathrm{kpc}$ (1--100 pc), and 50 in $R_{\mathrm{cut}}$. A persistent trend across these families is therefore difficult to attribute to a narrow parameter-tuning artefact.
...
\end{equation}
with $\epsilon$ the run softening length.
...
Using the shrinking-sphere center ensures that the innermost bins of the density profile are measured relative to a dynamically meaningful origin. In practice, this removes one of the most obvious numerical artefacts that could otherwise be mistaken for a central density decline. A formal description of the algorithm and the exact analysis implementation are given in Appendix~\ref{app:shrinking_sphere}.
...
\begin{figure}[H]
		\centering
		\includegraphics[width=1\textwidth]{content/ch4/img/truncated_nfw_technical.png}
...
\end{equation}
with $\varepsilon_m=10^{-10}$ for masked dark-matter sources.
...
\end{equation}
where $\mathbf{a}_{\mathrm{res}}$ is the resolved tree/pair/mesh contribution and $\mathbf{a}_{\mathrm{ext}}$ is the analytic truncated-NFW acceleration. In parallel, the source term entering the gravity solver is replaced by
\begin{equation}
m_{\mathrm{eff}}(\mathbf{x}_s) = m_s\,w_{\mathrm{src}}(\mathbf{x}_s),
\end{equation}
so the tree and multipole moments are built from the masked mass field rather than from the raw particle masses.

This notation captures the key implementation detail that the pseudocode notes explicitly: masked dark-matter particles are not assigned exactly zero weight, but a tiny non-zero factor $\varepsilon_m=10^{-10}$ to avoid degeneracies in the tree and multipole code paths. Thus, for a dark-matter particle $p$ one may write
\begin{equation}
m_{\mathrm{eff},p} = m_p\left[\chi_{\mathrm{in}}(\mathbf{x}_p) + \varepsilon_m\,\chi_{\mathrm{out}}(\mathbf{x}_p)\right],
\end{equation}
while all non-dark-matter species retain their original masses. The target-side rule is equally sharp: for $\chi_{\mathrm{out}}(\mathbf{x}_t)=1$, resolved contributions from the tree, pair and mesh solvers are suppressed at write-back time, and only the analytic field contributes to the particle acceleration and external potential bookkeeping.

\subsection{Two-layer masking logic}

The gravity split is enforced through two complementary mechanisms: source-side masking (particles outside the cutoff contribute negligibly to resolved sources) and target-side suppression (outer particles do not receive resolved write-backs). The final repository implementation follows a conservative, low‑overhead strategy that differs slightly from an earlier interaction-time-only design.

\textbf{Source-side masking.} To avoid expensive per-step global rewrites, the code now performs a one-time startup/restart masking sweep that sets the stored particle mass for eligible dark-matter particles to a tiny non-zero value. Concretely, a \texttt{mass\_initial} field was added to the \texttt{gpart} layout (see \texttt{src/gravity/*/gravity\_part.h}), and the helper
\texttt{gravity\_sync\_truncated\_nfw\_masses(struct gpart *gparts, size\_t)} is invoked at initialization and after restart (see \texttt{src/engine.c}). This helper uses the same geometric cutoff test implemented by
\texttt{gravity\_is\_outside\_truncated\_nfw\_cutoff(const double x[3])} and applies
\(m_{\rm stored} = m_{\rm initial}\times\varepsilon_m\) for dark‑matter particles with $r>R_{\rm cut}$, where $\varepsilon_m\approx10^{-10}$. After this one-time pass, the gravity code reads the stored \texttt{gp->mass} (and packed/foreign mass fields) directly during multipole builds, pair interactions and mesh depositions.

Note: the earlier helper \texttt{gravity\_get\_effective\_mass\_from\_position()} still exists in the codebase for diagnostic and transitional purposes, but the production runtime uses the stored masked mass following the one‑time initialization sweep. Attempts to restore or resync masses every step (or per drift) were investigated but removed because they produced unacceptable O(N) costs in typical runs.

\textbf{Target-side suppression.} The implementation still prevents outer particles from receiving resolved write-backs: the geometric predicate \texttt{gravity\_is\_outside\_truncated\_nfw\_cutoff(...)} is used to gate the write-back paths so that, for targets with $r>R_{\rm cut}$, tree/pair/mesh contributions are not added to the particle acceleration or potential. This predicate is used in the same places as originally designed:
...
The behaviour above can be expressed in a more detailed, step‑by‑step pseudocode that mirrors the actual runtime flow and clarifies the interactions between local data, packed/foreign frames, and the external backend.
...
epsilon_m = 1e-10   # masked mass fraction
...
Explanation: this sweep replaces the earlier idea of recomputing effective mass at every interaction with a single, explicit overwrite of the stored mass for outer DM particles. It is cheap to read later (no per-step writes) but is an O(N) operation performed only at startup or after a restart.

\item \textbf{MPI packing / foreign consistency}
\begin{lstlisting}[basicstyle=\footnotesize\ttfamily,breaklines=true]
# when creating the MPI packed structure for a particle:
packed.mass = gp.mass   # export the stored (possibly masked) mass directly
# receiver uses packed.mass directly when building multipoles / pair lists
\end{lstlisting}

Explanation: exporting the stored mass (not a mask factor) guarantees identical multipole/source semantics on all ranks and avoids branch divergence or reconstruction logic on the receiver.
...
Notes: the external backend computes analytic acceleration and potential for outer particles; an additive potential constant may be applied to keep the potential continuous at $R_{\rm cut}$ (this only affects reported energies, not the force).

\item \textbf{Diagnostics and energy bookkeeping}
\begin{lstlisting}[basicstyle=\footnotesize\ttfamily,breaklines=true]
E_pot_res = sum_i gparts[i].mass * Phi_res(gparts[i].x)
E_pot_ext = sum_i gparts[i].mass * Phi_ext(gparts[i].x)
# report both channels separately and the expected non-conservative term
\end{lstlisting}

This separation allows validation of the hybrid split: inner resolved energy vs outer analytic energy.
...
For diagnostics, the implementation effectively tracks two energy-like sums,
\begin{equation}
