...
Usage examples:
python3 plotMass.py snapshot_0000.hdf5 snapshot_0010.hdf5 --rmin 0.01 --rmax 10 --nbins 100 --log xy --types dm --center-mode shrinking -o mass_plot.png
...
def load_particle_set(handle, type_name, pos_scale, mass_scale):
    ...
    if "Masses" in g:
        print(f"{type_name}: using per-particle masses")
        mass = np.asarray(g["Masses"]) * mass_scale
    else:
        mt = get_mass_table(handle)
        print(f"{type_name}: using MassTable value {mt[idx]}")
...
def radial_profile(pos, mass, center, rmin, rmax, nbins, logbins):
    ...
    mask = (r >= 4.999) & (r <= 5.001)

    print(f"\nParticles:")
    print(f"Number of particles: {np.sum(mask)}")

    for i, m in enumerate(mass[mask]):
        print(f"Particle {i}: mass = {m}")
