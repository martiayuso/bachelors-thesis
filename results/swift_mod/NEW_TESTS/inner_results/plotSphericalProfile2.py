...
def MakePlot(opt):
  ...
  for filename in opt.files:
    ...
    def fix_particle_masses(nb, m_fix=6.164273e+02, thresh=1e-15):
        mass = nb.Mass(units="Msol").copy()
        mass[mass < thresh] = m_fix
        nb.mass = mass
        return nb
    ...
    if opt.y == 'density':
      ...
      mass_fixed = nb.mass.copy()
      mass_fixed[mass_fixed < 1e-15] = m_part

      # set labels
      xlabel = r'$\rm{Radius}\,\left[ \rm{kpc} \right]$'
      ylabel = r'$\rm{Density}\,\left[ M_\odot / kpc^3 \right]$'
      ...
      for j in range(opt.nlos):
        ...
        mass_per_bin = G.get_MassMap(nbc)

        # Get the mass of a single particle (assuming all particles are same type/mass)
        m_part = nbc.mass[0]

        # Calculate counts
        counts = mass_per_bin / m_part
        
        print(nb.mass)

        print("---------------------------------------------------------")
        print("bin analysis")
        print(f"  Particles in innermost bin: {counts[0]:.2f}")
        print(f"  Particles in outermost bin: {counts[-1]:.2f}")
        print(f"  Total particles within rmax: {np.sum(counts):.0f}")
        print("---------------------------------------------------------")
    ...
    if opt.y == 'mass':
      ...
      # Fix pathological tiny particle masses
      nb.mass[nb.mass < 1e-15] = 6.164273e+02
  ...
  save_name = f"profile_{os.path.basename(filename)}.npz"
  np.savez(save_name, radius=x, density=y)
  print(f"Saved profile data to {save_name}")  
